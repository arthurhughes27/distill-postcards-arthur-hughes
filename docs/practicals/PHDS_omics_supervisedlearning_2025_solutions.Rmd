...
```{r}
# Set the axis limits as the range of the predictions and response
results_range <- range(c(cv_results$response, cv_results$predicted))

# Make a little limit_paddingding on the range to avoid points on the extreme edges of the plot
limit_padding <- diff(results_range) * 0.02
lims <-
  c(results_range[1] - limit_padding, results_range[2] + limit_padding)

# Bonus - annotate the plot with the metrics
x_pos <- lims[2] - 0.02 * diff(lims)
y_pos <- lims[2] - 0.02 * diff(lims)
label_txt <- sprintf("Std RMSE = %.3f\nR² = %.3f", std_rmse, r2)

# plot
ggplot(cv_results, aes(x = response, y = predicted)) +
  ...
  labs(title = "True vs Predicted values (Ridge regression)",
       x = "True (observed)",
       y = "Predicted") +
  annotate(
    "text",
...
```{r}
# Select the counts only
counts = df_all %>% 
  dplyr::select(a1cf:zzz3)

# Function to find the names of the most variable predictors
mostvariable_predictors = function(counts, threshold_proportion) {
  
  # Make sure counts are numeric by converting to a matrix
  counts = counts %>%
    as.matrix()
  
  # Find the variances of each gene
  gene_vars = colVars(counts)
  
  # Put this information into a data frame, sorted by variance
  gene_vars_df = data.frame(
    "gene" = names(gene_vars),
    "var" = gene_vars
  ) %>%
    arrange(desc(var))
  
  # take the top n% of these genes
  genes2keep = gene_vars_df %>%
    head(threshold_proportion * nrow(gene_vars_df)) %>%
    pull(gene)
...
```{r}
# This gives the genes names of the top 10% of genes by variance
genes2keep = mostvariable_predictors(counts, 0.1)

# Select only the top 10% of predictors by variance
df_predict = df_all %>% 
  dplyr::select(response, all_of(genes2keep))
...
ggplot(cv_results, aes(x = response, y = predicted)) +
  ...
  coord_equal(xlim = lims, ylim = lims) +
  labs(title = "True vs Predicted values (Ridge regression, filtered variables)",
       x = "True (observed)",
       y = "Predicted") +
  annotate(
    "text",
...
ggplot(cv_results, aes(x = response, y = predicted)) +
  ...
  coord_equal(xlim = lims, ylim = lims) +
  labs(title = "True vs Predicted values (Lasso regression, filtered)",
       x = "True (observed)",
       y = "Predicted") +
  annotate(
    "text",
...
# Make prediction dataframe by selecting response and predictors (filtered genes)
df_predict <- df_all %>% dplyr::select(response, genes2keep)
...
n_inner <- 5
# Lambda values to consider
lambda_grid <- c(0.001, 0.005, 0.01, 0.05, 0.1, 0.5, 1, 2)
...
for (i in seq_along(folds_outer)) { 
  ...
  message(sprintf("Finished outer fold %d: selected lambda = %g, selected alpha = %g, test RMSE = %0.4f",
                  i, best_lambda, best_alpha, test_rmse))
}
...
ggplot(cv_results, aes(x = response, y = predicted)) +
  ...
  coord_equal(xlim = lims, ylim = lims) +
  labs(title = "True vs Predicted values (Elastic net regression)",
       x = "True (observed)",
       y = "Predicted") +
  annotate(
    "text",
...
# In this solution, I use the filtered dataframe
genes2keep = mostvariable_predictors(counts, 0.1)
...
# Tuning grid for PCR: number of principal components to consider
ncomp_grid <- c(seq(1,10,1),seq(15,50,5))  
...
# We define a "tuning grid" for PCR: ncomp only
tune_grid <- expand.grid(ncomp = ncomp_grid)
...
for (i in seq_along(folds_outer)) { 
  ...
  # caret::train() will:
  # - center & scale (preProcess),
  # - perform inner CV to choose best ncomp in tune_grid
  # - return the model fitted on the full training set using the best tune
  fit <- train(
    x = X_train,
    y = y_train,
  ...
  # Extract the best tuning parameter (number of components)
  ...
  outer_results[[i]] <- list(
    ...
    resample_results = fit$results,   # rmse for each ncomp from inner CV
    test_rmse = test_rmse,
    y_test = y_test,
    y_pred = preds
  )
  
  message(sprintf("Finished outer fold %d: selected ncomp = %d, test RMSE = %0.4f",
                  i, best_ncomp, test_rmse))
}
...
# Make a little limit_padding padding on the range to avoid points on the extreme edges of the plot
limit_padding <- diff(results_range) * 0.02
lims <-
  c(results_range[1] - limit_padding, results_range[2] + limit_padding)
...
# plot (updated title to indicate PCR)
ggplot(cv_results, aes(x = response, y = predicted)) +
  geom_point(alpha = 0.7, size = 2) +
  geom_abline(
    slope = 1,
  ...
  coord_equal(xlim = lims, ylim = lims) +
  labs(title = "True vs Predicted values (Principal component regression, filtered data)",
       x = "True (observed)",
       y = "Predicted") +
  annotate(
    "text",
...
# Tuning grid for PLS: number of components to consider
ncomp_grid <- c(seq(1,10,1),seq(15,50,5))  
...
# We define a "tuning grid" for PLS: ncomp only
tune_grid <- expand.grid(ncomp = ncomp_grid)
...
for (i in seq_along(folds_outer)) { 
  ...
  fit <- train(
    x = X_train,
    y = y_train,
    method = "pls",                     # <<-- PLS via pls package
    preProcess = c("center", "scale"),
    tuneGrid = tune_grid,  
    trControl = inner_ctrl,
    metric = "RMSE"
...
# plot (updated title to indicate PLS)
ggplot(cv_results, aes(x = response, y = predicted)) +
  geom_point(alpha = 0.7, size = 2) +
  geom_abline(
    slope = 1,
  ...
  coord_equal(xlim = lims, ylim = lims) +
  labs(title = "True vs Predicted values (PLS regression, filtered data)",
       x = "True (observed)",
       y = "Predicted") +
  annotate(
    "text",
...
# Grid of hyperparameter values (alpha is always = 1 in lasso regression)
tune_grid <- expand.grid(alpha = 1, lambda = lambda_grid)

# Number of CV loops to tune hyperparameters
n_inner = 5
...
# Extract the best lambda value from the final model
best_lambda <- final_model$lambdaOpt

# Extract the coefficients of the final model
## The output of the coef() function is not particularly nice, we will have to manipulate this a bit
coefs <- coef(final_model, s = best_lambda)

# First convert to a matrix
coef_mat <- as.matrix(coefs) 

# Now extract coefficient information into a dataframe
coefs_df <- data.frame(
  name = rownames(coef_mat),
  coefficient = as.numeric(coef_mat[,1]),
  row.names = NULL,
  stringsAsFactors = FALSE
)

# Now find the names of the genes with non-zero coefficients
lasso_gene_signature = coefs_df %>% 
  filter(coefficient != 0) %>% 
  pull(name)

# How many genes were selected?
message(paste0(lasso_gene_signature %>% length(), " genes were selected."))

# What are the gene names? 
lasso_gene_signature

# Perhaps we could extract these gene names as a comma separated string and input them into a software which can find the biological functions which they represent 
# For example DAVID bioinformatics, even generative AI tools can give a quick idea

# This code pastes the gene names (minus the intercept) and separates them by commas
paste(lasso_gene_signature[-1], collapse = ", ")
```
...
# Note : this line is a workaround for a known error in mixomics in the regression setting with 0 values in the response vector
y_vec = y_vec + 0.000001

# X as numeric matrix
X_df <- df_predict %>% dplyr::select(-response)

# Define the hyperparameter grid
keepX_grid <- c(seq(1,10,1),seq(10,50,5))

# Tuning
tune_result <- tune.spls(
  X = X_df,
  Y = y_vec,             
  ncomp = 5,
...
best_ncomp = tune_result$choice.ncomp[[1]]

best_keepX = tune_result$choice.keepX[best_ncomp]
```
...
```{r}
spls_final <- spls(X_df, y_vec, keepX = best_keepX, ncomp = best_ncomp) 

# What are the gene names? 
sPLS_gene_signature = selectVar(spls_final, comp = 1)$X$name 

paste(sPLS_gene_signature, collapse = ", ")

plotLoadings(spls_final, method = 'mean', contrib = 'max') 
```
...
# Alpha values to consider - note that we do not consider alpha = 0 (ridge regression), as this is not capable of variable selection.
...
for (i in seq_along(folds_outer)) { 
  ...
  # coef() returns a sparse matrix with rownames = "(Intercept)" and predictor names
  coef_sparse <- coef(fit$finalModel, s = best_lambda)
  coef_mat <- as.matrix(coef_sparse)
  coef_vec <- as.numeric(coef_mat[, 1])
  names(coef_vec) <- rownames(coef_mat)
  # predictors with non-zero coefficients (exclude intercept)
  selected_predictors <- setdiff(names(coef_vec)[which(coef_vec != 0)], "(Intercept)")
  # -------------------------------------------------------------------------
  ...
  outer_results[[i]] <- list(
    ...
    y_test = y_test,
    y_pred = preds,
    selected_predictors = selected_predictors   # <<-- stored here
  )
  
  message(sprintf("Finished outer fold %d: selected lambda = %g, selected alpha = %g, test RMSE = %0.4f (n selected = %d)",
                  i, best_lambda, best_alpha, test_rmse, length(selected_predictors)))
}
...
p1 <- ggplot(cv_results, aes(x = response, y = predicted)) +
  geom_point(alpha = 0.7, size = 2) +
  geom_abline(
    slope = 1,
    intercept = 0,
...
all_predictors <- colnames(X_df)

# initialize counts
sel_counts <- setNames(rep(0L, length(all_predictors)), all_predictors)
# For loop to count how many times each predictor was selected
for (i in seq_along(outer_results)) {
  sel <- outer_results[[i]]$selected_predictors
  if (!is.null(sel) && length(sel) > 0) {
    sel_counts[sel] <- sel_counts[sel] + 1L
  }
}
# Make a dataframe with the selection proportion results
sel_prop_df <- data.frame(
  predictor = names(sel_counts),
  prop = as.numeric(sel_counts) / n_outer,
  count = as.integer(sel_counts),
  stringsAsFactors = FALSE
)

# keep only those selected more than 50% of the time
sel_prop_df <- sel_prop_df %>% dplyr::filter(prop > 0.5)

# sort from most to least frequently selected
sel_prop_df <- sel_prop_df %>% dplyr::arrange(dplyr::desc(prop))

# plot proportion selected
p2 <- ggplot(sel_prop_df, aes(x = reorder(predictor, desc(prop)), y = prop)) +
    geom_col() +
    labs(x = "Gene",
         y = "Proportion of outer folds selected",
         title = "Selection frequency across outer folds (>50% selection)") +
    theme_minimal(base_size = 18) +
    theme(
